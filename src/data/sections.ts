...
茎の節から太く伸びてくる **{{term:気根}}** は、自生地で他の木に絡んで体を支えたり、空気中や樹皮の水分を取り込んだりするための根です。室内栽培でもよく伸びてきますが、「切っていいの？」と迷う方が多いポイントです。
...
- **支柱に誘導して活かす**：ヘゴ棒や水苔ポールに気根を絡ませると、上方向に安定して仕立てられ、葉も大きく育ちやすくなります（仕立ての詳細は [季節管理・剪定・増やし方](/monstera-info/seasonal/) を参照）。
- **長い気根は土や水へ**：伸びた気根の先を鉢の用土や水を入れた小容器に導くと、水分吸収を助けられます。

⚠️ 気根を無理に曲げたり結束したりすると折れることがあります。折れても株が枯れることはありませんが、誘導するときはゆるやかに添える程度にします。
...
`,
  ...
  },
  {
    ...
    description: '春夏秋冬それぞれの管理ポイント、剪定で節（ふし）を守るコツ、水苔密閉法・水挿し・土挿しの3つの増やし方と、発根しないときの対処までを解説します。',
    icon: 'scissors',
    emoji: '✂️',
...
節を含む茎を水や土に挿す考え方は、モンステラ以外の観葉植物でも共通します。たとえばガジュマルも[挿し木や水挿しで増やせます](/gajumaru-info/propagation/)。
...
| 症状 | よくある原因 | 対処 |
|---|---|---|
| いつまでも発根しない | 節を含んでいない／気温が低い（20°C未満） | 必ず節を含む茎に切り直し、暖かい明るい日陰で養生する |
| 切り口や茎が黒く腐る | 水の汚れ・溶存酸素不足、水苔や土の過湿 | 水挿しは毎日水換え、培地挿しは過湿を避けて清潔に保つ |
| 葉だけ挿して根が出ない | 葉のみで節がない | 葉一枚だけでは発根・生長しない。節と気根を含む茎を使う |
| 発根後に新葉が出ない | 定植が早すぎる／日照不足 | 根が3〜5cmに育ってから定植し、明るい場所で管理する |

⚠️ 「早く育てたい」と発根前から肥料を与えるのは逆効果です。肥料は根が十分に育って定植したあとから、規定濃度で少量ずつ始めます。
...
- **寒さのサイン**：葉が黒っぽく変色したり、株元が柔らかくなってきたら低温障害の可能性があります。詳しくは [病害虫・トラブル対処](/monstera-info/troubles/) を参照してください。

💡 暖房の風が直接当たる場所は、葉が乾燥して傷んだり、ハダニが発生しやすくなります。エアコンの送風ルートを避けて置きましょう。

## 屋外で育てている株の取り込み

春から秋にベランダや庭で育てている場合は、**朝晩が冷え込み始める前**に室内へ取り込みます。最低気温が **10〜15℃** を下回るようになったら、取り込みのサインです（地域差があるため、お住まいの地域の気温を目安にしてください）。
...
{{figure:variegation-patterns}}

### 安定性は系統によって大きく違う（価格差の根拠）

| 系統 | 遺伝的背景 | 組織培養での再現性 | 価格傾向 |
|---|---|---|---|
| **タイ・コンステレーション** | **斑が安定して受け継がれる系統** | 組織培養で再現可能、斑が安定 | 量産可能で比較的入手しやすい |
| **アルボ（Albo）** | **キメラ斑**（体細胞モザイク） | 組織培養で安定再現不可、節の状態次第 | 希少・高価、緑戻り/致死性白化リスク大 |
| **オーレア** | **キメラ斑** | 同上、不安定 | 希少 |
| **ミント斑** | 不明、希少変異 | 安定性低い | 極めて希少 |

タイ・コンステレーションの斑は **安定して受け継がれる** ため、組織培養で増やしても斑が再現されると考えられています。一方アルボなどの斑は、白色変異細胞と緑色細胞が入り混じった **キメラ（モザイク）** 状態とされ、新葉が出る節がどちらの細胞群を多く引き継ぐかで斑の入り方が変わります。植物学では、葉の最外層（L1層）に変異がそろった「周縁キメラ」は栄養繁殖で安定して受け継がれる一方、変異が一部だけの斑は受け継がれにくいことが知られており、これが系統ごとの安定性・価格差・入手難度の背景にあります。

📖 個々の斑入り系統の遺伝的な仕組みは、まだ研究途上で断定が難しい部分もあります。本サイトでは「組織培養で斑が安定して再現できるか」という実用面を中心に整理しています。
...
`,
    ...
    references: [
      ...
      { label: '周縁（ペリクリナル）キメラが栄養繁殖で安定して受け継がれる仕組み（斑の安定性の植物学的背景）', publisher: 'PMC / NCBI（査読論文）', url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC4223383/' },
    ],
  },
  {
    id: 'safety',
...
観葉植物といっても、毒性の仕組みは種類によって異なります。たとえば同じく人気の[ガジュマル](/gajumaru-info/pet-safety/)（イチジク属）の刺激成分はシュウ酸カルシウムではなく、フィシンという別の酵素成分です。複数の植物を育てる場合は、それぞれの仕組みを分けて理解しておくと安全です。
